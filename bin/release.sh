...
for cmd in git gh jq ruby bundle; do
...
VERSION_FILE="lib/chartmogul/version.rb"
STASHED=false
...
CURRENT_VERSION=$(ruby -r "./lib/chartmogul/version" -e "puts ChartMogul::VERSION")
...
interrupt_ci_wait() {
  ...
  echo "  RubyGems: https://rubygems.org/gems/chartmogul-ruby/versions/${NEW_VERSION}"
  restore_stash
  exit 0
}
