...
module ChartMogul
  module CSV
    class SubscriptionEvent < Base
      SUBSCRIPTION_EVENT_HEADERS = %w[External\ ID Subscription\ set\ external\ ID Subscription\ external\ ID Customer\ external\ ID Plan\ external\ ID Date Effective\ Date Event\ Type Currency Amount\ in\ Cents Quantity Retracted\ event\ ID].freeze
